...
--;===========================================================
--; ADVANCE LOOP (ARCADE, TEAM CO-OP, SURVIVAR, SURVIVAR CO-OP, VS 100 KUMITE, BOSS RUSH)
--;===========================================================
		...
		if p1Cell then
			if numChars ~= #data.t_p1selected then
				...
				if getCharName(p1Cell) == 'Random' then
					sndPlay(sysSnd, 100, 0)
					f_drawCharAnim(t_selChars[math.random(1, #t_randomChars)], 'p1AnimStand', 30 + 30*#data.t_p1selected, 150, updateAnim)
				else
					f_drawCharAnim(t_selChars[p1Cell+1], 'p1AnimStand', 30 + 30*#data.t_p1selected, 150, updateAnim)
				end
			end
			for j=#data.t_p1selected, 1, -1 do
				f_drawCharAnim(t_selChars[data.t_p1selected[j].cel+1], 'p1AnimStand', 30 + 30*(j-1), 150, data.t_p1selected[j].up)
			end
		end
		local nameX, nameY = f_drawSelectName(txt_p1Name, 4, data.t_p1selected, 10, 162, 0, 14)
		...
		if p2Cell then
			if numChars ~= #t_selected then
				...
				if getCharName(p2Cell) == 'Random' then
					sndPlay(sysSnd, 100, 0)
					f_drawCharAnim(t_selChars[math.random(1, #t_randomChars)], 'p2AnimStand', 290 - 30*#t_selected, 150, updateAnim)
				else
					f_drawCharAnim(t_selChars[p2Cell+1], 'p2AnimStand', 290 - 30*#t_selected, 150, updateAnim)
				end
			end
			for j=#t_selected, 1, -1 do
				f_drawCharAnim(t_selChars[t_selected[j].cel+1], 'p2AnimStand', 290 - 30*(j-1), 150, t_selected[j].up)
			end
		end
		local nameX, nameY = f_drawSelectName(txt_p2Name, 1, t_selected, 309, 162, 0, 14)
...
--; STAGE SELECT
--;===========================================================
txt_selStage = createTextImg(jgFnt, 0, 0, '', 160, 237)
...
200,0, 0,0, -1
		...
		while true do
			...
			f_drawSelectName(txt_p2NameVS, 0, data.t_p2selected, 241, 190, 0, 14, p2Row, 1)
			--draw match counter
			if matchNo > 0 then
